...
<form id="groupAddForm">
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="inputDelay" aria-describedby="inputDelayHelp" name="Delay" value="5.0">
...
$(document).ready(function() {
    $('#groupAddForm').submit(function(event) {
        ...
        $.post("api/v1/scene/{{.SceneID}}/group", formData, function(data) {
            if (data.Error != false) {
                alert("Error: " + data.Message);
            } else {
                $('#focusSelector').text('#groupRow{{.OrderNext}}')
                populateContent();
