...
<form method="POST" id="cycleForm">
    <div class="form-group">
        <label for="inputType">Scene</label>
        <select class="custom-select" class="form-control" id="inputScene" name="SceneID">
            <option value=""></option>
{{ range .Scenes }}
            <option value="{{.ID}}"{{if eq $.Cycle.SceneID .ID}} selected{{end}}>{{.Name}}</option>
{{ end }}
        </select>
    </div>
    <div class="form-group">
        <label for="inputCycles">Scene Cycles</label>
        <input type="text" class="form-control" id="inputCycles" aria-describedby="inputCyclesHelp" name="SceneCycles" value="{{.Cycle.SceneCycles}}">
        <small id="inputCyclesHelp" class="form-text text-muted">How many times to run the scene.</small>
    </div>
    <div class="form-group">
        <label for="inputName">End Delay</label>
        <input type="text" class="form-control" id="inputEndDelay" aria-describedby="inputEndDelayHelp" name="EndDelay" value="{{.Cycle.EndDelay}}">
    ...
    <div class="form-group">
        ...
        <select class="custom-select" class="form-control" id="inputLoopInclude" aria-describedby="inputLoopIncludeHelp" name="LoopInclude">
            <option value="true"{{if (eq .Cycle.LoopInclude true)}} selected{{end}}>true</option>
            <option value="false"{{if (eq .Cycle.LoopInclude false)}} selected{{end}}>false</option>
        </select>
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="inputGlobalParameterValue1" aria-describedby="inputGlobalParameterValue1Help" name="GlobalParameter1" value="{{.Cycle.GlobalParameter1}}">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="inputGlobalParameterValue2" aria-describedby="inputGlobalParameterValue2Help" name="GlobalParameter2" value="{{.Cycle.GlobalParameter2}}">
...
$(document).ready(function() {
    $('#cycleForm').submit(function(event) {        
        ...
        $.post("api/v1/show/{{.Cycle.ShowID}}/cycle/{{.Cycle.ID}}/edit", formData, function(data) {
            if (data.Error != false) {
                alert("Error: " + data.Message);
            } else {
                populateContent();
                $("#editCycleModal").dialog("close");
            }
        }, "json");
